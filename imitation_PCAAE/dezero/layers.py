...
class Layer:
    ...
    def save_weights(self, path, BackToTheGPU=False):
        self.to_cpu()
...
class Linear(Layer):
    ...
    def _init_b(self, xp=np):
        b_data = xp.zeros(self.out_size, dtype=self.dtype)
        self.b.data = b_data    
...
class Conv2d(Layer):
    ...
    def _init_b(self, xp=np):
        b_data = xp.zeros(self.out_channels, dtype=self.dtype)
        self.b.data = b_data
...
class BatchNorm(Layer):
    def __init__(self, eps=2e-5, decay=0.9, affine=True, track_running_stats=True):
        super().__init__()
        self.eps = eps
        self.decay = decay
        self.affine = affine
        ...
        else: # constants: avg mean = 0 & avg var = 1
            self.avg_mean = Parameter(None, name='avg_mean', constant=True)
            self.avg_var = Parameter(None, name='avg_var', constant=True)

        if self.affine:
            self.gamma = Parameter(None, name='gamma')
            self.beta = Parameter(None, name='beta')
        else: # constants: gamma = 1 & beta = 0
            self.gamma = Parameter(None, name='gamma', constant=True)
            self.beta = Parameter(None, name='beta', constant=True)

    def _init_MeanVar(self, x):
        xp = cuda.get_array_module(x)
        D = x.shape[1]
    ...
    def __call__(self, x):
        if self.avg_mean.data is None:
            self._init_MeanVar(x)
        if self.gamma.data is None:
            self._init_GammaBeta(x)
            
        return F.batch_nrom(x, self.gamma, self.beta, self.avg_mean.data, self.avg_var.data, decay=self.decay, eps=self.eps, affine=self.affine, track_running_stats=self.track_running_stats)
...
class Layer_stepby:
    ...
    def __setattr__(self, name, value):
        if isinstance(value, (Parameter, Layer, Layer_stepby)):
            self._params.add(name)
        elif isinstance(value, list):
            for i in range(len(value)):
                if isinstance(value[i], (Parameter, Layer, Layer_stepby)):
                    name_i = name + '_' + str(i)
                    self.__getattr__(name_i)
                    self.__setattr__(name_i, value[i])
        super().__setattr__(name, value)

    def __getattr__(self, name):
        setattr(self, name, None)
    
    def __call__(self, *inputs, step):
        outputs = self.forward(*inputs, step)
        if not isinstance(outputs, tuple):
            outputs = (outputs,)
    ...
    def params(self):
        for name in self._params:
            ...
            if isinstance(obj, (Layer, Layer_stepby)):
                yield from obj.params()
            else:
                yield obj
