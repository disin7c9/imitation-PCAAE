...
class BatchNorm(Function):
    def __init__(self, mean, var, decay, eps, affine, track_running_stats):
        self.avg_mean = mean
        self.avg_var = var
        self.decay = decay
        self.eps = eps
...
def batch_nrom(x, gamma, beta, mean, var, decay=0.9, eps=2e-5, affine=True, track_running_stats=True):
    return BatchNorm(mean, var, decay, eps, affine, track_running_stats)(x, gamma, beta)
...
# new function
# =============================================================================
# concatenate encodings with shape(batch, 1)
class Cat1Dcode(Function):
    def forward(self, *xs):
        xslist = []
        for x in xs:
            if x.ndim != 2 or x.shape[1] != 1:
                raise ValueError('{} has improper shape {}.'.format(x, x.shape))
            xslist.append(x)
        
        self.num_encs = len(xslist)
        xp = cuda.get_array_module(xslist[0])
        z = xp.concatenate(xslist, axis=-1)
        return z
    
    def backward(self, gy):
        gydata = gy.data
        gxs = [Variable(gydata[:, i].reshape(-1, 1)) for i in range(self.num_encs)]
        return (*gxs,)
